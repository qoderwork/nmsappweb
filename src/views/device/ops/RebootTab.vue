  ...
  <div class="reboot-tab">
    ...
    <el-table :data="taskList" v-loading="loading" border stripe>
      ...
      <el-table-column :label="t('common.actions')" width="280" fixed="right">
        <template #default="{ row }">
          <el-button size="small" type="primary" :icon="VideoPlay" @click="handleStart(row as RebootTaskVO)">{{ t('deviceOps.start') }}</el-button>
          <el-button size="small" type="warning" :icon="CircleClose" @click="handleCancel(row as RebootTaskVO)">{{ t('deviceOps.cancel') }}</el-button>
          <el-button size="small" :icon="View" @click="handleViewResults(row as RebootTaskVO)">{{ t('deviceOps.results') }}</el-button>
          <el-button size="small" type="danger" :icon="Delete" @click="handleDelete(row as RebootTaskVO)">{{ t('common.delete') }}</el-button>
        </template>
      </el-table-column>
    </el-table>
    <el-pagination
    ...
    <!-- Add dialog -->
    <el-dialog v-model="dialogVisible" :title="t('deviceOps.addRebootTask')" width="640px" @closed="resetForm">
      <el-form ref="formRef" :model="form" :rules="formRules" label-width="140px">
        ...
        <el-form-item :label="t('deviceOps.softReboot')">
          <el-switch v-model="form.softReboot" />
        </el-form-item>
      </el-form>
      <template #footer>
...
import type { RebootTaskVO, RebootTaskResultVO, AddRebootTaskRequest } from '@/types/device-ops';
...
const form = reactive<AddRebootTaskRequest>({
  name: '',
  executeMode: 1,
...
async function loadTasks() {
  loading.value = true;
  try {
    const page = await listRebootTasks({ ...query });
...
async function submitForm() {
  ...
  await formRef.value.validate(async (valid) => {
    ...
    try {
      const payload: AddRebootTaskRequest = {
        name: form.name,
        executeMode: form.executeMode,
        deviceType: form.deviceType,
        scope: form.scope,
        softReboot: form.softReboot,
        elementIds: [],
        deviceGroupIds: [],
      };
...
async function handleDelete(row: RebootTaskVO) {
  try {
...
async function handleStart(row: RebootTaskVO) {
  try {
    await startRebootTask(row.id);
...
async function handleCancel(row: RebootTaskVO) {
  try {
    await cancelRebootTask(row.id);
    ElMessage.success(t('common.operateSuccess'));
    await loadTasks();
  } catch (e: unknown) {
    const msg = e instanceof Error ? e.message : String(e);
...
async function handleViewResults(row: RebootTaskVO) {
...
async function loadResults() {
  ...
  try {
    const page = await listRebootTaskResults(currentTaskId.value, { ...resultQuery });
