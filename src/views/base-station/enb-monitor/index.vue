...
async function loadData() {
  loading.value = true;
  try {
    const res = await getDevices({ keyword: queryParams.keyword || undefined, page: queryParams.page, pageSize: queryParams.pageSize, device_type: 'enb' } as any);
    tableData.value = res.list; total.value = res.total;
  } catch (e) { console.error('[EnbMonitor] load failed:', e); }
...
<template>
  <div class="enb-monitor-page">
    <el-card shadow="never" :body-style="{ padding: '16px' }">
      <el-form :inline="true" @submit.prevent="handleSearch">
        <el-form-item>
          <el-input v-model="queryParams.keyword" placeholder="搜索序列号/设备名/IP" clearable style="width: 260px" @keyup.enter="handleSearch" @clear="handleSearch"><template #prefix><el-icon><Search /></el-icon></template></el-input>
        </el-form-item>
        <el-form-item>
...
.enb-monitor-page { display: flex; flex-direction: column; gap: var(--spacing-base, 12px); width: 100%; }
