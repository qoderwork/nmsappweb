...
import { getUpgradeTasks, createUpgradeTask, startUpgradeTask, getUpgradeResults } from '@/api/upgrade';
...
async function loadTasks() {
  loading.value = true;
  try {
    const res = await getUpgradeTasks({ keyword: queryParams.keyword || undefined, page: queryParams.page, pageSize: queryParams.pageSize, deviceType: 'gnb' });
    taskList.value = res.list; total.value = res.total;
  } catch (e) { console.error('[BsUpgrade] load failed:', e); }
...
async function handleAdd() {
  ...
  try {
    await createUpgradeTask({ name: addForm.name, execute_mode: addForm.executeMode, device_type: 'gnb', upgrade_file_id: addForm.upgradeFileId, scope: 'element', element_ids: addForm.elementIds.join(',') });
    ElMessage.success(t('common.addSuccess')); addDialogVisible.value = false; loadTasks();
  } catch (e) { console.error('[BsUpgrade] add failed:', e); }
}

async function handleStart(task: UpgradeTaskVo) { try { await startUpgradeTask(task.id); ElMessage.success(t('common.operateSuccess')); loadTasks(); } catch (e) { console.error('[BsUpgrade] start failed:', e); } }
...
async function openResultDialog(task: UpgradeTaskVo) {
  ...
  try { const res = await getUpgradeResults(task.id); resultList.value = (res || []) as UpgradeResultVo[]; } catch (e) { console.error('[BsUpgrade] results failed:', e); }
...
.bs-upgrade-page { display: flex; flex-direction: column; gap: var(--spacing-base, 12px); width: 100%; }
